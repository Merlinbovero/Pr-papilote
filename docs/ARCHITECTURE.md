...
| 3D            | Three.js (import dynamique, une seule page)                                                                                  | Test d'orientation spatiale — modèles glTF orientés selon l'attitude   |
...
**Dépendance `three` (Three.js) — justifiée.** Le test d'orientation spatiale (`/psychotechnique/orientation`) exige d'afficher un aéronef à une **attitude 3D arbitraire** (cap, assiette, inclinaison) générée à l'infini ; aucune banque d'images finie ne peut couvrir l'espace des attitudes, et un rendu SVG « 2,5D » a été jugé insuffisant. Three.js charge des modèles **glTF de libre réutilisation** (CC BY, registre `src/lib/models-3d.ts`, crédités sur `/credits-photos`) et les oriente en direct. La dépendance est **importée dynamiquement** dans le seul composant client concerné (`src/features/psychotech/orientation-test.tsx`) : elle est _code-splittée_ et n'alourdit aucune autre page. La logique pure (génération des attitudes, notation) reste sans dépendance dans `src/lib/psychotech/orientation.ts` (testée). Alternatives écartées : `<model-viewer>` (embarque Three.js, plus lourd), rendu SVG à plat (foreshortening non convaincant), banque d'images pré-rendues (finie, lourde, non « infinie »).
...
### 2026-07-09 — Gestion documentaire et qualité éditoriale (Volume II, ch. 8)

Chaîne éditoriale consacrée (docs/editorial/gestion-documentaire.md) : le contenu est l'actif principal, chaque décision technique protège sa qualité, sa pérennité, sa cohérence. Consécration de l'existant (workflow 6 statuts, sources hiérarchisées, entité image, niveaux de lecture, références croisées du graphe, archivage vs suppression) et quatre ajouts.

(1) **Historique & motif** — champ `revisions[]` au contrat (date, version, motif, author, reviewer?) ; Git reste la vérité des diffs, ce registre porte le motif, affiché sur la fiche (RevisionHistory) ; intégrité : dernière version = `version`, versions uniques, dates croissantes. (2) **État éditorial unifié** — `editorialState(fiche, today)` → a-jour / a-verifier / mise-a-jour-necessaire (statut × fraîcheur), source unique pilotant le badge de la fiche. (3) **Notice de document consultable** — champ `summary` obligatoire, chargeur getDocuments/getDocument avec intégrité `relations.documents` (erreur bloquante au chargement), route `/documents/[id]` et composant NoticeDocument (consultation sur site ; lien officiel et téléchargement s'ajoutent, ne remplacent pas), prévisualisation /design-system/document. (4) **Contrôle qualité outillé** — `npm run content:check` (src/lib/content/content-check.ts, pur et testé) : erreurs (document manquant, vérification future) et avertissements (fraîcheur dépassée, fiche publiée sans quiz, document rediffusable sans binaire) ; branché sur la CI (étape dédiée) et couvert par `npm run check`. Chargeur de questions ajouté (getQuestions, intégrité `évalue`). Évolutivité (multilingue, variantes par concours) : chemin réservé sans champ mort (langue fr implicite, variantes par ID, schemaVersion).

### 2026-07-09 — Performance, accessibilité et qualité technique (Volume II, ch. 9)
