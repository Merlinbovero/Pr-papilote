...
| `MainNav`        | `components/layout/main-nav.tsx`        | Navigation principale : six sections (EOPAN, EOPN, ALAT, Fondamentaux, Psychotechnique, Culture) en menus déroulants ; tiroir dépliable sous `lg`. Structure dans `src/lib/navigation.ts` | —                                                                   |
| `SiteFooter`     | `components/layout/site-footer.tsx`     | Pied de page sobre : liens vers les six modules                                                                                                                                           | —                                                                   |
...
| `DominoTile`     | `features/psychotech/domino-tile.tsx`   | Domino dessiné en SVG (points en disposition de dé, blanc = 0) ; `DominoSeries` place une série entière et trace les liens de lecture, `DominoHalfTile` sert au pavé de saisie            | `domino`, `size?`, `tone?`, `label?`                                |
| `SceneImage`     | `features/psychotech/camera-scene.tsx`  | Rendu 3D d'une scène du test des appareils photos (Three.js dynamique, primitives calculées) : vue d'ensemble avec appareils numérotés, ou vue prise par un objectif                      | `puzzle`, `kind`, `alt`, `className?`                               |
| `ScenePlan`      | `features/psychotech/camera-plan.tsx`   | Plan de la scène vu de dessus en SVG : objets stylisés, appareils numérotés et cônes de visée                                                                                             | `puzzle`, `size?`                                                   |
| `SearchCommand`  | `features/search/search-command.tsx`    | Palette de recherche globale (Ctrl/Cmd+K), filtrage par notre moteur. `variant` : `hero` (barre de l'accueil), `compact` (bouton-champ), `icon` (bouton carré du header)                  | `entries: SearchEntry[]`                                            |
