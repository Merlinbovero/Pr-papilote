...
2. **Rédiger** sur le gabarit officiel (`gabarit-fiche.md`) : L'essentiel + À retenir, sections (strates _approfondir_ / _maîtriser_), pièges, sources.
   - **Schémas obligatoires dès que leur absence pénalise la compréhension** : les schémas font partie intégrante de la pédagogie. Ils sont **produits en même temps que la fiche** (on ne « repasse » jamais dessus des mois plus tard), sous forme de **SVG originaux, sobres, homogènes au design system et libres de droits** (`content/schemas/`, insérés en ligne, traits en `currentColor` adaptés au thème). Contrat : champ `figures[]` par section (`schemaId`, `alt`, `caption?`, dimensions). Tout schéma suit la **convention graphique homogène** (`convention-schemas.md`) : une même grandeur physique garde la même représentation partout.
